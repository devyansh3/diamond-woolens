...
import SalesAgentDrawer from "./SalesAgentDrawer";
...
function SalesAgentTable() {
    ...
    const fetchAgents = async () => {
        try {
            const response = await axios.get("http://localhost:2001/salesagents");
                    ...
                    <div className="h-full max-h-[calc(100vh-100px)] overflow-y-auto p-2 text-lg">
                        <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden h-full">
                            <div className="mx-4 flex items-center justify-between py-2">
                                <h5 className="dark:text-white font-semibold">Sales Agents</h5>
                                <button
                                ...
                                <div className="overflow-y-auto h-full pr-4">
                                    <Table striped className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                        <Table.Head className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                            ...
                                            <Table.HeadCell>Agent Phone</Table.HeadCell>
                                            <Table.HeadCell>
                                        ...
                                        <Table.Body>
                                            {filteredAgents?.map((agent, index) => (
                                                <Table.Row key={index} className="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                                    ...
                                                    <Table.Cell className="px-4 py-2 whitespace-nowrap text-xs text-gray-900 dark:text-white">
                                                        <button onClick={() => openEditDrawer(agent)} className="text-primary-700 hover:underline">
                                                            Edit
                                                        </button>
                                                    </Table.Cell>
                                                </Table.Row>
                                    ...
                                    {filteredAgents?.length === 0 && (
                                        <div className="text-center py-4">
                                            <p className="text-gray-500 dark:text-gray-400">No sales agents found.</p>
                                        </div>
                                    )}
                                </div>
                            </div>
